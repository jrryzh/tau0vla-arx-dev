...
Source episodes 0–25 (26 episodes) produce 9,383 frames at 30 FPS.
...
29 anchors per episode, leaving 8,629 training samples. With 16 H200s,
batch 8 per GPU, accumulation 1 and incomplete batches dropped, 6k steps
are approximately 89.55 data epochs; verify actual `vla_epoch` in logs.

This configuration has its own dataset registration, environment override
(`ARX_PICKANDPLACE_01_LEROBOT_ROOT`), unified 40D statistics and outputs.
...
python tools/convert_official_hdf5_to_lerobot.py \
  --input data/0904_pickandplace_01 --start 0 --end 25 \
  --source-fps 60 --fps 30 --action-mode state_t_plus_1 \
  --task "Pick up the tool and place it into the tray." \
  --output data/0904_pickandplace_01/lerobot_v3_30fps_state_t_plus_1 \
  --repo-id arx_lift2s/0904_pickandplace_01_30fps_state_t_plus_1
python scripts/validate_arx_lerobot_conversion.py data/0904_pickandplace_01/lerobot_v3_30fps_state_t_plus_1 \
  --source data/0904_pickandplace_01 --expected-episodes 26 --expected-frames 9383 \
...
PYTHONPATH=src:. python scripts/norm_stats/compute_unified_ft_stats.py \
  --body arx_lift2s_unified --repos data/0904_pickandplace_01/lerobot_v3_30fps_state_t_plus_1 \
  --action-horizon 30 --positive-labels --negative-labels \
  --partials-dir outputs/0904_pickandplace_preparation/stats_01
PYTHONPATH=src:. python scripts/norm_stats/merge_stats.py \
  --partials outputs/0904_pickandplace_preparation/stats_01 --out configs/arx_lift2s_pickandplace_01/norm_stats.json
```

## Launch and monitor

```bash
scripts/qzcli_arx_h200.sh auto --profile pickandplace-01 --credentials /secure/qzcli.txt
```
...
Model: `outputs/arx_lift2s_pickandplace_01_h200_formal/arx_lift2s_pickandplace_01_h200_formal/checkpoint-6000`.
Controller evidence: `outputs/qzcli_arx_h200_pickandplace_01/`.
